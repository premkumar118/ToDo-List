...
const getData = async () => {
  try {
    const data = await fetch("https://prem-todo-list.vercel.app/api/lists", {
      cache: "no-store",
    });
  ...
  return (
    <>
      {lists.map((list) => (
        <div
          key={list._id}
          className="flex justify-between border border-slate-950 px-5 py-3 items-start my-5 gap-5"
        >
          <div>
